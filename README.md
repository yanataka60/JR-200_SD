...
　A=USR($8003):REM xxxxxのように直後にREM文を書くと文字列をファイル名として解釈します。ダブルコーテーションは不要です。また、事前にLauncherを起動しておく必要もありません。

　LOAD出来るのは機械語プログラムのみです。

　BASICプログラムを指定するとLOADしBASICに復帰した時にBASICプログラムを破壊してしまい暴走します。
